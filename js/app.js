...
(function () {
  ...
  /* ---------- AI 解析：调用服务端代理（DeepSeek-V3）流式讲解题目 ---------- */
  function aiExplainIt(it, userAns, box) {
    if (!box) return;
    if (box.getAttribute('data-running') === '1') return;
    var btn = box.querySelector('.ai-explain-btn');
    ...
    var optsText = (it.options || []).map(function (o, i) { return letters[i] + '. ' + o; }).join('\n');
    var correctText = ansLetters(it.answer);
    var userText = userAns ? (userAns === correctText ? '（我选对了）' : '（我选了 ' + userAns + ' ，答错了）') : '（未作答）';
    var sys = '你是「天天滚动」西综考研网站的 AI 解析助手，帮助医学生理解西医综合考研真题。请用简洁、有条理的中文解析，结构：①考点 ②正确选项分析 ③干扰项排除（简要） ④一句话总结。正文不超过 350 字，不要客套。';
    var userMsg = '题目（' + it.subject + '）：\n' + it.question +
      '\n选项：\n' + optsText +
      '\n正确答案：' + correctText +
      '\n我的答案：' + userText +
      (it.explain ? '\n\n官方解析供参考：\n' + it.explain : '') +
  ...
  function bindAiExplain(it, userAns, box) {
    var btn = document.createElement('button');
    btn.className = 'ai-explain-btn';
    btn.textContent = '🤖 AI 解析';
    btn.addEventListener('click', function () { aiExplainIt(it, userAns, box); });
    box.appendChild(btn);
  }

  /* ---------- 对应导图片段（题目文本经 OCR 索引定位） ---------- */
  ...
  function findImgSnippet(it) {
    ...
    for (var i = 0; i < CARDS.length; i++) {
      var card = CARDS[i];
      if (card.subject !== it.subject) continue;
          ...
          : chapStats.map(o => `
            <div class="subj-row" data-subject="${esc(ps)}" data-chapter="${esc(o.chapter)}">
              <div class="subj-row-name">
                <div class="subj-name">${esc(cleanChapter(o.chapter))}</div>
        ...
        <div class="img-ch-group-body">
          ${groups[sub].map(c => `
            <label class="img-ch-row">
              ...
              <span class="img-ch-name">${esc(cleanChapter(c.chapter))}</span>
    ...
    return `
      <div class="learn-card" style="padding-bottom:0">
        <div class="learn-meta">
          ...
          ${it.chapter ? `<span class="tag sub">${esc(cleanChapter(it.chapter))}</span>` : ''}
          ${typeTag}${stateTag}
        </div>
  ...
  function renderExamResult() {
    ...
    $$('.exam-wrong').forEach((el, i) => {
      const w = ex.wrong[i];
      if (!w) return;
      const btn = document.createElement('button');
      btn.className = 'ai-explain-btn';
      btn.textContent = '🤖 AI 解析';
      btn.addEventListener('click', () => aiExplainIt(w.item, ansLetters(w.chosen), el));
      bindImgSnippet(w.item, el);
      el.appendChild(btn);
    });
      ...
      el.innerHTML = subjects.map(sub => {
        ...
        const chapHtml = chapters.map(ch => `
          <div class="lib-group-chapter">
            <div class="lib-group-chapter-title">${esc(cleanChapter(ch))}<span class="lib-group-count">${groups[sub][ch].length}</span></div>
...
// 历年真题卷 - 事件委托绑定交卷（核心修复，动态DOM不会丢失绑定）
let paperSubmitting = false;
document.querySelector("#examContainer").addEventListener("click", async function (e) {
  if (e.target.matches("#submitPaperBtn")) {
...
async function submitPaperExam() {
  ...
  try {
    ...
    confirmDom.style = `position:fixed;inset:0;background:rgba(0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:9999;`;
    ...
    console.error("交卷函数报错：", err);
    alert("交卷发生异常，查看F12控制台详情");
  } finally {
    // 无论成功失败，释放提交锁，按钮不会永久卡死
